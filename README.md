...
### `-mt`/`--metadata`: Metadata file(s) (optional)
  ...
  Ideally this will be the tsv files generated by `validate_sample_meta.pl` of [cgpNgsQc](https://github.com/cancerit/cgpNgsQc) when validating BAM's metadata. Besides the columns required by the tool ([more details](https://github.com/cancerit/cgpNgsQc/wiki#validate_sample_metapl)), a metafile used here can have `Sequencer` and `Sequencing year` columns. The tool will search samples names extracted from BAS files (e.g. sample name in a BAM file header) in both `Sample_ID` and `Sample_UUID` columns in the metadata files to map samples' metadata to their QC metrics.
